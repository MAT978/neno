...
class NenoHelper
{
	 ...
	 * @return  bool
	 */
	public static function generateIssue($code, $item, $extension, $opt)
	{
		...
		$query
			->select($db->quoteName(array('id', 'item_id', 'error_code', 'fixed')))
		...
		if ($db->getNumRows() == 0)
		{
			...
			$query
				...
				->columns(array('discovered', 'error_code', 'item_id', 'extension', 'info'))
				->values('NOW(), ' . $db->quote($code) . ', ' . $db->quote($item) . ', ' . $db->quote($extension) . ', ' . $db->quote($info));
		...
		else
		{
			...
			if ($issue->error_code != $code)
			{
				...
				if ($result)
				{
					$result = self::generateIssue($code, $item, $extension, $opt);
				}
			}
		}
