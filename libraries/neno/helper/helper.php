...
class NenoHelper
{
	...
	{
		$enGbExtensionIds = self::getEnGbExtensionIds();
		...
		if (!empty($enGbExtensionIds))
		{
			foreach ($enGbExtensionIds as $enGbExtensionId)
			{
				...
				$languagesFound = array_merge($updates, $languagesFound);
			}
		}
	 ...
	 * @since 1.0
	 */
	protected static function getEnGbExtensionIds()
	{
		$db       = JFactory::getDbo();
		$extQuery = $db->getQuery(true);
		$extType  = array('language', 'package');
		$extElem  = array('en-GB', 'pkg_en-GB');
