 ...
 */
class NenoFilterAliasInterfaceLegacy implements NenoFilterAliasInterface
{
	/**
	 * Character map
	 *
	...
	private function init($language = '')
	{
		...
		{
			foreach ($map as $orig => $conversion)
			{
				$this->map[$orig] = $conversion;
				$this->chars .= $orig;
			}
		}
	 ...
	 * @since 2.1.15
	 */
	protected function downcode($text, $language = '')
	{
		$this->init($language);
	 ...
	 * @since 2.1.15
	 */
	protected function filter($text, $language = '', $fileName = false)
	{
		$text = $this->downcode($text, $language);

		// if downcode doesn't hit, the char will be stripped here
		$removePattern = ($fileName) ? '/[^_\-.\-a-zA-Z0-9\s]/u' : '/[^\s_\-a-zA-Z0-9]/u';
		$text          = preg_replace($removePattern, '', $text); // remove unneeded chars
		...
		return trim($text, '-');     // trim to first $length chars
	}
